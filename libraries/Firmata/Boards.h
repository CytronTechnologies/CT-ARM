...
#define PIN_SERIAL3_RX          A4
#define PIN_SERIAL3_TX          A5
#define IS_PIN_ANALOG(p)        ((p) >= A0 && (p) < A0 + TOTAL_ANALOG_PINS)
#define IS_PIN_DIGITAL(p)       (((p) >= 2 && (p) < A0 + TOTAL_ANALOG_PINS))
#define IS_PIN_PWM(p)           (((p) >= 2 && (p) <= A3))
#define IS_PIN_SERVO(p)         (((p) >= 2 && (p) <= A3))
#define IS_PIN_I2C(p)           ((p) == SDA || (p) == SCL)
#define IS_PIN_SPI(p)           ((p) == SS || (p) == MOSI || (p) == MISO || (p) == SCK)
#define IS_PIN_INTERRUPT(p)     IS_PIN_DIGITAL(p)
#ifdef PIN_SERIAL1_RX && PIN_SERIAL1_TX
...
#else
#define IS_PIN_SERIAL(p)        ((p) == PIN_SERIAL2_RX || (p) == PIN_SERIAL2_TX || (p) == PIN_SERIAL3_RX || (p) == PIN_SERIAL3_TX)
#endif
#define PIN_TO_DIGITAL(p)       (p)
#define PIN_TO_ANALOG(p)        ((p) - A0)
#define PIN_TO_PWM(p)           (p)
...
unsigned char readPort(byte port, byte bitmask)
{
  ...
  /*unsigned char out = 0, i = 0, pin = port * 8;
  for (i = 0; i < 8; i++) {
    if (PIN_TO_DIGITAL(pin + i) && (bitmask & (1 << i)) && digitalRead(PIN_TO_DIGITAL(pin + i))) {
      out |= (1 << i);
    }
  }*/
