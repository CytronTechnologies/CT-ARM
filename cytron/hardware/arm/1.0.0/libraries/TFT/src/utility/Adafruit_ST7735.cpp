...
inline void Adafruit_ST7735::spiwrite(uint8_t c) {
  ...
  if (hwSPI) {
    ...
    for(uint8_t bit = 0x80; bit; bit >>= 1) {
      if(c & bit) dataport->DOUT |=  datapinmask;
      else        dataport->DOUT &= ~datapinmask;
      clkport->DOUT |=  clkpinmask;
      clkport->DOUT &= ~clkpinmask;
    }
  }
}
...
#endif
  rsport->DOUT &= ~rspinmask;
  csport->DOUT &= ~cspinmask;
...
void Adafruit_ST7735::commonInit(const uint8_t *cmdList) {
  ...
  if(hwSPI) { // Using hardware SPI
	...
	SPI.setClockDivider(0); //no divider, original speed is already 4MHz
#else
